...
@timer
def test_entropy_average_time(
    ave_num=3, cutoff=5.0, sigma=0.2, kind="cpu", cal_ovito=True, check_ovito=False
):
    time_list = []
    print("*" * 30)
    ...
    if kind == "cpu":
        np.savetxt(
            "time_list_cpu_entropy.txt",
            time_list,
            delimiter=" ",
            header="N ovito mdapy",
        )
    elif kind == "gpu":
        np.savetxt(
            "time_list_gpu_entropy.txt",
            time_list,
            delimiter=" ",
            header="N ovito mdapy",
        )
...
def plot(time_list, title=None, kind="cpu", save_fig=True):
    ...
    if save_fig:
        plt.savefig(
            "entropy_mdapy_ovito.png", dpi=300, bbox_inches="tight", transparent=True
        )
    # plt.show()
...
if __name__ == "__main__":
    ...
    time_list_gpu = test_entropy_average_time(
        ave_num=3, cutoff=5.0, sigma=0.2, kind="gpu", cal_ovito=True, check_ovito=False
    )

    mp.init("cpu")
    time_list_cpu = test_entropy_average_time(
        ave_num=3, cutoff=5.0, sigma=0.2, kind="cpu", cal_ovito=False, check_ovito=False
    )
    time_list_cpu = np.loadtxt("time_list_cpu_entropy.txt")
    time_list_gpu = np.loadtxt("time_list_gpu_entropy.txt")
    ...
    plot(time_list, title="Calculate entropy", kind="cpu-gpu", save_fig=True)
