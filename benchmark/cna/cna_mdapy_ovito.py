...
def mdapy_cna(pos, box, rc = 3.615*0.86):
    ...
    neigh.compute()
    cna = mp.CommonNeighborAnalysis(neigh.rc, neigh.verlet_list, neigh.neighbor_number, pos,box)
    cna.compute()
...
def test_cna_average_time(ave_num=3):
    ...
    for num in [5, 25, 45, 65, 85, 105, 125]: # 5, 25, 45, 65, 85, 105
        ...
        for turn in range(ave_num):
            ...
            start = time()
            data.apply(CommonNeighborAnalysisModifier(mode=CommonNeighborAnalysisModifier.Mode.FixedCutoff, cutoff=3.615*0.86))
            end = time()
            ovito_t += end - start
                
        time_list.append([N, mdapy_t_cpu / ave_num, mdapy_t_gpu / ave_num, ovito_t/ave_num])
        print("*" * 30)
