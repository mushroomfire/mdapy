...
def mdapy_pair(pos, box, N,):
    rho = N/np.product(box[:,1]-box[:,0])
    neigh = mp.Neighbor(pos=pos, box=box, rc=5., max_neigh=90) 
    neigh.compute()
    pair = mp.PairDistribution(neigh.rc, 200, rho,neigh.verlet_list, neigh.distance_list, neigh.neighbor_number)
    pair.compute()
...
def pair_average_time(ave_num=3):
    ...
    for num in [5, 25, 45, 65, 85, 105, 125]: # 5, 25, 45, 65, 85, 105
        np.random.seed(10)
        N = num*40000
        pos = np.random.random((N, 3))*np.array([num*3.615, 100*3.615, 100*3.615])
        ...
        for turn in range(ave_num):
            ...
            f_box = freud.box.Box(
                    Lx=box[0, 1], Ly=box[1, 1], Lz=box[2, 1]
                )
            shift_pos = pos - np.min(pos, axis=0) - (box[:, 1]) / 2
            start = time()
            rdf = freud.density.RDF(200, 5.)
        ...
        time_list.append([N, mdapy_t_cpu / ave_num, mdapy_t_gpu / ave_num, ovito_t/ave_num, freud_t/ave_num])
        print("*" * 30)
