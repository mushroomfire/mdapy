...
class AtomicStrain:
    ...
    def __init__(self, ref_pos, ref_box, cur_pos, cur_box, verlet_list, neighbor_number, boundary=[1, 1, 1], affi_map='off') -> None:
    ...
    @ti.kernel
    def _cal_atomic_strain(self, verlet_list:ti.types.ndarray(), 
        ...
        identy = ti.Matrix([[1., 0., 0.], [0., 1., 0.], [0., 0., 1.]], dt=ti.f64)
        for i in range(N):
            V = ti.Matrix([[0.]*3 for _ in range(3)], dt=ti.f64)
            W = V
            for jj in range(neighbor_number[i]):
                j = verlet_list[i, jj]
                delta_ref = _pbc(ref_pos[j] - ref_pos[i], self.boundary, ref_box, ref_box_inv)
                delta_cur = _pbc(cur_pos[j] - cur_pos[i], self.boundary, cur_box, cur_box_inv)
            ...
            s = (F.transpose() @ F - identy)/2.
            xydiff = s[0, 0] - s[1, 1]
            yzdiff = s[1, 1] - s[2, 2]
            xzdiff = s[0, 0] - s[2, 2]
            shearStrain = ti.sqrt(s[0, 1]**2 + s[0, 2]**2 + s[1, 2]**2 +
    ...
    def compute(self):
        ...
        self._cal_atomic_strain(self.verlet_list, 
