...
def mdapy_neigh(pos, box, rc=5., max_neigh=50):
    neigh = mp.Neighbor(pos=pos, box=box, rc=rc, max_neigh=max_neigh) 
    neigh.compute()

def test_neighbor_thread_time(ave_num=3, rc=5.):
...
def plot(time_list_cpu, time_list_gpu, title=None, savefig = True):
	...
	for i, j in enumerate([25, 50, 75, 100, 125]):
	    ...
	    plt.text(0.14, data[0, 2], r"$\mathregular{%d \times 10^6}$ atoms" % (j*100*100*4/1000000))
	...
	if savefig:
		plt.savefig('neighbor_thread.png', dpi=300, bbox_inches='tight', transparent=True)
	plt.show()
