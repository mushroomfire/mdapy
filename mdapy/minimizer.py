...
class Minimizer:
    ...
    def __init__(self, pos, box, boundary, potential, elements_list, type_list, mini_type='FIRE', fmax=0.05, max_itre=100) -> None:
        self.pos = pos 
    ...
    def compute(self):
        ...
        for step in range(self.max_itre):
            pe_atom, force, _ = self.potential.compute(self.pos, self.box, self.elements_list, self.type_list, self.boundary)
            pe = pe_atom.sum()
...
if __name__=='__main__':
    ...
    pe_atom, _, _ = potential.compute(lat.pos, lat.box, [element_name], lat.type_list, [1, 1, 1])
    E_bulk = pe_atom.sum()
    print(f"Bulk energy: {E_bulk:.2f} eV.")

    mini_defect = Minimizer(
        np.ascontiguousarray(lat.pos[1:]), lat.box, [1, 1, 1],
        potential, [element_name], np.ascontiguousarray(lat.type_list[1:]),
        fmax=fmax,
        max_itre=max_itre
    )
    mini_defect.compute()
