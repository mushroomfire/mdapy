...
def test_cluster_average_time(ave_num=3, check_ovito=False):
    ...
    for num in [5, 25, 45, 65, 85, 105, 125]:
        ...
        for turn in range(ave_num):
            ...
            start = time()
            system.cal_cluster_analysis(rc=5.0)
            end = time()
            mdapy_t += end - start
            if check_ovito:
                print(f"Checking results of {turn} turn...")
                assert (
                    system.data["cluster_id"].values
                    == np.array(data.particles["Cluster"][...])
                ).all()
    ...
    np.savetxt(
        "time_list_cpu_cluster.txt", time_list, delimiter=" ", header="N ovito mdapy"
    )
    return time_list
...
def plot(time_list, title=None, kind="cpu", save_fig=True):
    ...
    if save_fig:
        plt.savefig(
            "cluster_mdapy_ovito.png", dpi=300, bbox_inches="tight", transparent=True
        )
    # plt.show()
