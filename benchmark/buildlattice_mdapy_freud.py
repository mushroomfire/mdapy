    ...
    for num in [5, 10, 15, 20, 25, 30, 50, 70, 100, 150, 200, 250]:
        ...
        for turn in range(ave_num):
            ...
            if check:
                ...
                assert np.allclose(
                    np.min(shift_pos, axis=0), np.min(FCC.pos, axis=0)
                ) & np.allclose(np.max(shift_pos, axis=0), np.max(FCC.pos, axis=0))
        time_list.append(
...
if __name__ == "__main__":
    ...
    mp.init("cpu")
    # time_list_cpu = test_neighbor_average_time(ave_num=3, kind='cpu', check=False)
