    ...
    for num in [5, 25, 45, 65, 85, 105, 125]:
        ...
        for turn in range(ave_num):
            ...
            if check:
                ...
                assert (
                    np.abs(np.min(shift_pos, axis=0) - np.min(FCC.pos, axis=0)).max()
                    < 1e-3
                )
                assert (
                    np.abs(np.max(shift_pos, axis=0) - np.max(FCC.pos, axis=0)).max()
                    < 1e-3
                )
        time_list.append(
...
if __name__ == "__main__":
    ...
    time_list_gpu = test_lattice_average_time(
