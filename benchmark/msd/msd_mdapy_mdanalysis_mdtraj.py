...
def get_u_mdanalysis(N, pos_list):
    ...
    u = MDAnalysis.Universe(topology, 'MSD.gsd', dt=1.)
    return u
...
def msd_average_time(ave_num=3):
    ...
    for N in [100, 500, 1000, 2500, 5000, 7500, 10000]: 
        ...
        pos_list = np.cumsum(np.random.choice([-1., 1.], size=(Nframe, N, 3)), axis=0) * np.sqrt(2) # Generate a random walk data
        ...
        for turn in range(ave_num):
            ...
            start = time()
            MDAnalysis.analysis.msd.EinsteinMSD(u, select='all', msd_type='xyz', fft=True).run()
            end = time()
        ...
        time_list.append([N, mdapy_t_cpu / ave_num, mdapy_single_t/ave_num, mdanalysis_t/ave_num, mdtraj_t/ave_num])
        print("*" * 30)
