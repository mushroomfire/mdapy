subroutine build_cell(N, Dim, bin_l, Box_Size, Position, Atom_Cell_List, &
    Cell_Id_List, n_thread, a, b, c) 
    use omp_lib
    implicit none

    integer (kind = 8) :: i, icel, jcel, kcel
    integer ( kind = 8 ), intent(in) :: N, Dim, a, b, c, n_thread
    ...
    !$omp parallel &
    !$omp shared ( Position, N, xmin, ymin, zmin, bin_l, ncel, Atom_Cell_List, Cell_Id_List) &
    !$omp private ( i, icel, jcel, kcel )
    !$omp do
    do i = 0, N-1
...
subroutine build_verlet_list_cell(Position, Atom_Cell_List, Cell_Id_List, &
    Verlet_List, bin_l, Rangesqure, N, Dim, &
    Pbc_boundary, Box_Size, n_thread, a, b, c, max_neigh)
    use omp_lib
    implicit none
    integer (kind = 8) :: i, j, icel, jcel, kcel, nneiindex, nnei
    integer (kind = 8) :: iicel, jjcel, kkcel, iiicel, jjjcel, kkkcel
    integer ( kind = 8 ), intent(in) :: N, Dim, a, b, c, n_thread, max_neigh 
    ...
    integer(kind = 8), intent(inout), dimension(0:N-1, 0:max_neigh-1) :: Verlet_List
    ...
    nneiindex = 0
    !$omp parallel &
    !$omp shared(Position, N, xmin, ymin, zmin, bin_l, ncel, Atom_Cell_List, Cell_Id_List, Verlet_List) &
    !$omp private(i, j, icel, jcel, kcel, iicel, jjcel, kkcel, iiicel, jjjcel, kkkcel, Rij, Rsqij, nnei)
    !$omp do
    do i = 0, N-1
        nnei = 0
        icel = floor((Position(i, 0)-xmin)/bin_l)
            ...
            do jjcel = jcel-1, jcel+1
                do kkcel = kcel-1, kcel+1
                    ...
                    do while (j > 0) 
                        ...
                        Rsqij = sum(Rij**2)
                        if ((Rsqij < Rangesqure) .and. (j /= i)) then
                            Verlet_List(i, nnei) = j
                            nnei = nnei + 1
                        end if 
                        j = Atom_Cell_List(j)
                    end do
        ...
        if (i > 0) then
            Rij = position(i, :) - position(0, :)
            call pbc(Rij, Pbc_Boundary, Box_Size, Dim)
            Rsqij = sum(Rij**2)
            if (Rsqij < Rangesqure) then
                Verlet_List(i, nnei) = 0
                nnei = nnei + 1
            end if 
        end if 
    nnei = 0
